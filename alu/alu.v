...
module alu #(parameter N = 16, parameter modes = 4) (
	input[N - 1:0] a,
	input[N - 1:0] b,
	...
	one_comp #(.N(N)) not_block(.a(b), .comp_a(one_x), .cout(not_stat[2]), .z(not_stat[0]), .n(not_stat[1]), .o(not_stat[3]));
	inc #(.N(N)) incr(.a(a), .x(inc_x), .cout(inc_stat[2]), .z(inc_stat[0]), .n(inc_stat[1]), .o(inc_stat[3]));
	dec #(.N(N)) decr(.a(a), .x(dec_x), .cout(dec_stat[2]), .z(dec_stat[0]), .n(dec_stat[1]), .o(dec_stat[3]));
